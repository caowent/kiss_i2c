...
/* 主节点写单字节数据 */
uint8_t msp_i2c_write_byte(uint8_t dev, uint16_t addr, uint8_t byte)
{
    return msp_i2c_write_bytes(dev, addr, ADDR_IS_8b, &byte, 1);
}

/* 主节点读单字节数据 */
uint8_t msp_i2c_read_byte(uint8_t dev, uint16_t addr)
{
    uint8_t byte;

    msp_i2c_read_bytes(dev, addr, ADDR_IS_8b, &byte, 1);
